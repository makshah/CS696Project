...
def cross_entropy_loss2d(inputs, targets, cuda=False, balance=1.1):
    ...
    loss = nn.BCELoss(weights, reduction='sum')(inputs,
                                                targets)  # loss=nn.BCELoss(weights,size_average=False)(inputs,targets)
    return loss
...
def train(model, args):
    ...
    for step in range(start_step, args.max_iter + 1):
        ...
        for i in range(args.iter_size):
            ...
            pic = pic.convert('L')
            pic.save(os.path.join("./results", 'fuse/images/train', '%s.png' % loss.item()), "PNG")
...
def parse_args():
    ...
    parser.add_argument('--max-iter', type=int, default=40200,
